...
const SERVICES = [
  {
    ...
    cardClassName: "min-h-[430px] sm:p-10",
  },
  {
  ...
  },
  {
    ...
    cardClassName: "min-h-[240px] lg:min-h-0",
  },
];
...
export default function Services() {
  return (
    <SectionShell id="services" noPadding className="pt-44 pb-24 lg:pt-52 lg:pb-32">
      <SectionHeading
        eyebrow="What we do"
      ...
      <motion.div
        ...
        className="mt-24 grid auto-rows-[minmax(220px,auto)] gap-6 lg:mt-28 md:grid-cols-2 lg:grid-cols-6"
      >
        {SERVICES.map((service) => (
          ...
          >
            <BentoCard className={`flex h-full flex-col p-8 ${service.cardClassName}`}>
              ...
              <h3 className={`${fraunces.className} mt-6 max-w-xl text-[2rem] leading-tight text-[#241913] dark:text-[#fff7ec] ${service.title === "Full-funnel community growth" ? "sm:text-[2.6rem]" : ""}`}>
                {service.title}
              </h3>
              <p className="theme-copy mt-4 max-w-2xl text-[15px] leading-relaxed">
                {service.desc}
              </p>
              <div className={`mt-8 flex-1 ${service.title === "Precision ads management" ? "grid gap-3 sm:grid-cols-3" : "space-y-4"}`}>
                {service.features.map((feature) => (
                  <div
                    key={feature}
                    className="flex items-center gap-3 rounded-full border border-[#eadccd] bg-[#fff8ef]/70 px-4 py-2 text-[15px] font-medium text-[#5f4c3d] dark:border-[#3a2c21] dark:bg-[#19130f]/70 dark:text-[#d9ccb9]"
                  >
